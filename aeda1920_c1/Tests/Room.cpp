...
bool Room::operator<(const Room &room) const{
    return code < room.code || (code == room.code && floor < room.floor);
}
...
float Room::price() const{
    return (float)roomWidth * (float)roomLength * 4.0 / 10000;
}
float LuxuryRoom::price() const {
    return (float)getWidth() * (float)getLength() * 4.0 * 1.15 /10000;
}
