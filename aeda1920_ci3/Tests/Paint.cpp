...
bool operator==(Paint* first, const Paint& second){
    return first->getAuthor() == second.getAuthor() && first->getTitle() == second.getTitle();
}
...
//TODO
bool PaintCatalogItem::operator<(const PaintCatalogItem& pci) const {
    return getYear() > pci.getYear() || (getYear() == pci.getYear() && getAuthor() < pci.getAuthor()) || (getYear() == pci.getYear() && getAuthor() == pci.getAuthor() && getTitle() < pci.getTitle());
}
